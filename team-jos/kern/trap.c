...
trap_init(void)
{
	...
	//cprintf("MY SYSCALL setting %p to %p\n", &idt[T_SYSCALL], syscallhandler);
	//SETGATE(idt[T_SYSCALL], 1, GD_KT, syscallhandler, 0);
	// Per-CPU setup
	//}
	trap_init_percpu();
}
...
trap_dispatch(struct Trapframe *tf)
{
	...
	case T_SYSCALL:
		...
		if(tf->tf_regs.reg_eax != -E_INVAL) {
			// valid system call, return to calling user code
			return;
		} else {
			// oh no! the user code invoked an unknown
			// system call.  We'll just treat that like
			// any other unexpected trap and panic
		}
	}
