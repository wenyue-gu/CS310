...
#define STACK_MEM_START 0xf9f8c000
#define STACK_SIZE 1024*64


int main_fixedstack(int argc, char** argv) {
...
This is the code that fixes the stack at a particular position in
memory.  It's not a normal thing for a process to do - generally
speaking even on very similar systems the stack will be in a slightly
different place.  So actual exploits will need to explore the memory
space a bit and use a sled to get their exploit code running.  Then
with ASLR it'll be in a different position with every run - even
harder.

Having you do that sort of thing would make this exercise more
realistic, but it would also make verifying that your exploit works
much more annoying.  So for everybody's ease this main ensures that
the stack always starts in exactly the same place.

 */
int main(int argc, char *argv[]) {

  void* memory = mmap((void*) STACK_MEM_START,
                      STACK_SIZE,
  ...
  context.uc_link = 0;
  context.uc_stack.ss_sp = (void*) STACK_MEM_START;
  context.uc_stack.ss_size = STACK_SIZE - 128;
  context.uc_stack.ss_flags = 0;        
 
  // Create the new context
  printf( "Creating child thread\n" );
  makecontext( &context, (void (*)(void)) main_fixedstack, 2, (void*) argc, (void*) argv);
  setcontext(&context);
  perror("failed to switch contexts");
  exit(1);
}
