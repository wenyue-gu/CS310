...
void test_uint32_equal(os_uint32_t a, os_uint32_t expected,
                       char *testname, os_uint32_t ignored) {
  printf(" - '%s' == %lu  ", testname, (unsigned long int) expected);
  if (a == expected) {
    printf("OK");
  } else {
      printf("failed! (was %d)", a);
  }
  printf(" [Test %d]\n", ++(*num_tests));
...
void test_string_equal(char *a, char *expected,
  ...
  if (strcmp(a, expected) == 0) {
    printf("OK");
      ...
      printf("failed! (was \"%s\")", a);
  }
  printf(" [Test %d]\n", ++(*num_tests));
}

#define TEST_FINISHED 99
...
int main(int argc, char **argv) {
  ...
  // map the num_tests variable as shared memory
  num_tests = mmap(NULL,
                   sizeof(num_tests),
  ...
  // you could uncomment this if you want but it makes running the
  // code in a debugger more annoying
  ...
  if(fork_result != 0) {
      // I'm the parent so I'm just checking for a crash
      int wait_result;
      wait(&wait_result);
      if(WEXITSTATUS(wait_result) == TEST_FINISHED) {
          printf("tester finished running successfully\n");
      } else {
          printf("test runner crashed.  tests after %d assumed a failure.\n", *num_tests);
      }
      return 0;
  } // else execute the tests
  */
