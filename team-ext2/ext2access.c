...
os_bool_t fetch_inode(os_uint32_t inode_number, int fd,
  ...
  os_uint32_t blockgroup_num = inode_number/metadata->inodes_per_group;  // you must fix this.
//5
//2
//12,34,5
  ...
  // the 10th?  the 100th?)
  os_uint32_t offset_within_blockgroup = inode_number - (blockgroup_num) * metadata->inodes_per_group-1;  // you must fix this.
  //printf("inode num %d  inode per group %d block group num %d offset in block group %d \n",inode_number, metadata->inodes_per_group, blockgroup_num, offset_within_blockgroup);
  ...
  if(offset_within_blockgroup>metadata->blockgroup_size){
    return FALSE;
  }
  ...
  os_uint32_t offset1 = offset_within_blockgroup - inode_block_num*num_inodes_per_block;
  inode_block_num +=
  ...
  os_uint32_t offsetnum =inode_block_num * metadata->block_size + offset1 * sizeof(struct os_inode_t);
  //= inode_block_num * metadata->block_size + offset_within_blockgroup * sizeof(struct os_inode_t);
  //offset_within_blockgroup - num_inodes_per_block * inode_block_num;
  //printf("num inode per block %d  inode block num %d offset1 %d offset %d",num_inodes_per_block, inode_block_num, offset1, offsetnum);
  ...
  lseek(fd, (off_t)offsetnum, SEEK_SET);
...
void calculate_offsets(os_uint32_t blocknum,
  ...
  if (blocks_left < (blocksize/4)*(blocksize/4)*(blocksize/4)) {
    ...
    printf("%d\n",blocks_left);
    *direct_num = -1;
    *triple_index = floor(blocks_left/((blocksize/4)*(blocksize/4)));
    blocks_left = blocks_left % ((blocksize/4)*(blocksize/4));
